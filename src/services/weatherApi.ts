    ...
    if (error.response) {
      switch (error.response.status) {
        ...
        case 500:
          ...
          break;
        default:
          errorMessage = error.response.data.message || "An error occurred while fetching weather data.";
          break;
      }
    }
    throw new ApiError(errorMessage, error.response?.data, error.response?.status);
  } else {
